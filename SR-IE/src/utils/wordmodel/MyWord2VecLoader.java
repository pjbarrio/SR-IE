	...
	public Word2Vec loadModel(String path,boolean binary, Set<String> wordSet, boolean normalized) throws IOException {
		...
		if(binary) {
			...
			try {
				...
				for (int i = 0; i < words*2; i++) { //words*2 because of spaces.
					
					if (i % 1000000 == 0)
						System.out.println("Loaded " + i/2 + " of " + words + " - " + wordsTotal);
	...
	public Set<String> loadDictionary(String path, boolean binary) throws IOException {
		...
		if(binary) {
			...
			try {
				...
				for (int i = 0; i < words*2; i++) { //I need to multiply because there are whitespaces...
					
					if (i % 1000000 == 0){
						System.out.println("Loaded: " + i*50.0 / (double)words);
					}
