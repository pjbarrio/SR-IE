...
public class OutputCombinerSplits {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
		...
		Map<String,Set<String>> mapExpts = (Map<String,Set<String>>)SerializationHelper.deserialize(which == null?"exp_summarize.summary":which+"_exp_summarize.summary");
		

		for (Entry<String, Set<String>> exp_files : mapExpts.entrySet()) {

			if (!exp_files.getKey().equals(which))
				continue;


			String outp_file = output_folder + "splits." + exp_files.getKey();
			...
			String hdr = "perc,data,relation,extractor,model,modelsize,modelphrase,target,targetsummary,targetsize,sample,split";
			...
			for (String prefix : exp_files.getValue()) {
				...
				if (relation.equals("PersonLocation")){
					continue;
				}
				
				for (double perc = 10.0; perc <= 90.0; perc+=10.0) {

					Set<Integer> candidates = null;
					
					try{
						candidates = loadSplit(data,extractor,relation,split,perc);	
					}catch(IOException e){
						e.printStackTrace();
						break;
					}
					...
					String prf = perc + "," + data + "," + relation + "," + extractor + "," + model + "," + modelsize + "," + modelphrase + "," + target +
					...
					System.out.println("\n" + f.getAbsolutePath());
					bw.newLine();
					bw.write(prf + 0 + ",0.0001,0.9,"+sents+",0,-1,-1,0," + totaluseful);
					...
					while (f.exists()){
						...
						while ((line = br.readLine()) != null){
							...
							int cand = Integer.valueOf(line.split(",")[4].trim());
							...
							if (candidates.contains(cand)){
								...
								if (line.endsWith("1")){ //is useful
									totaluseful++;
									bw2.newLine();
									bw2.write(prf + position + "," +  line.replace(" ", ""));
								}
	...
	private static Set<Integer> loadSplit(String data, String extractor,
			String relation, String split, double perc) throws IOException {

		List<String> splits = FileUtils.readLines(new File(getPrefix(data)+"splits/"+getFullExtractor(extractor)+"."+getFullRelation(relation)+"."+perc+"."+split+".sample"));

		Set<Integer> set = new HashSet<Integer>(splits.size());
	...
	private static String getFullRelation(String relation) {

		if ("Man Made Disaster-Location".equals(relation)){
			return "ManMadeDisaster";
		}else if ("PersonParty".equals(relation)){
			return "PersonParty";
		}else if ("CompanyCustomer".equals(relation)){
			return "CompanyCustomer";
		}

		throw new UnsupportedArchitectureException("No Relation in" + relation);
	...
	private static String getFullExtractor(String extractor) {

		if ("BONG".equals(extractor)){
			return "Pablo-N-Grams";
		}else if ("OC".equals(extractor)){
			return "OpenCalais";
		}

		throw new UnsupportedArchitectureException("No Extractor in " + extractor);

	}

	private static String getPrefix(String data) {
		if ("Train".equals(data)){
			return "data/train/";
		}else if ("LTrain".equals(data)){
			return "data/ltrain/";
		}

		throw new UnsupportedArchitectureException("No Data in " + data);
	}
